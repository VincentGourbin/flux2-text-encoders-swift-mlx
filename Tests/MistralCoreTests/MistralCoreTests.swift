 ...
 * Main unit tests for MistralCore library
 */
...
final class MistralCoreTests: XCTestCase {
    ...
    func testVersion() {
        XCTAssertFalse(MistralCore.version.isEmpty, "Version should not be empty")
        XCTAssertEqual(MistralCore.version, "0.1.0", "Version should be 0.1.0")
    }
    ...
    func testMistralCoreSharedInstance() {
        let core1 = MistralCore.shared
        let core2 = MistralCore.shared
    ...
    func testMistralCoreErrorDescriptions() {
        XCTAssertEqual(MistralCoreError.modelNotLoaded.errorDescription,
                      "Model not loaded. Call loadModel() first.")

        XCTAssertEqual(MistralCoreError.vlmNotLoaded.errorDescription,
                      "VLM not loaded. Call loadVLMModel() first for vision capabilities.")

        let invalidInput = MistralCoreError.invalidInput("test message")
        XCTAssertEqual(invalidInput.errorDescription, "Invalid input: test message")

        let genFailed = MistralCoreError.generationFailed("gen error")
    ...
    func testGenerateThrowsWhenModelNotLoaded() {
        ...
        XCTAssertNotNil(MistralCoreError.modelNotLoaded.errorDescription)
    }
    ...
    func testAnalyzeImageThrowsWhenVLMNotLoaded() {
        XCTAssertNotNil(MistralCoreError.vlmNotLoaded.errorDescription)
    }
...
final class MistralCoreIntegrationTests: XCTestCase {
