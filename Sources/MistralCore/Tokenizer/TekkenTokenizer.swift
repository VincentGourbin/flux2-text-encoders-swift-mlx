...
public class TekkenTokenizer {
    ...
    public func loadTekkenTokenizerFromFile(modelPath: String) {
        ...
        guard let jsonData = try? Data(contentsOf: URL(fileURLWithPath: tekkenPath)) else {
            MistralDebug.log("Cannot load \(tekkenPath), trying HuggingFace tokenizer format...")
        ...
        do {
            ...
            if let specialTokens = tekkenVocab.special_tokens {
                ...
                MistralDebug.log("Loaded \(specialTokens.count) special tokens")
            }

            // Load additional config from generation_config.json
            loadSpecialTokens(modelPath: modelPath)

            MistralDebug.log("Loaded tokenizer with \(mergeableRanks.count) tokens, numSpecialTokens=\(numSpecialTokens)")

        } catch {
            MistralDebug.error("Error parsing Tekken JSON: \(error)")
            loadDefaultTokenizer()
        }
    }
    ...
    private func loadHuggingFaceTokenizer(modelPath: String) {
        ...
        if let tokenizer = result.tokenizer {
            ...
            MistralDebug.log("Loaded HuggingFace tokenizer successfully")
        } else {
            if let error = result.error {
                MistralDebug.error("Failed to load HuggingFace tokenizer: \(error)")
            }
            MistralDebug.log("Falling back to default tokenizer")
            loadDefaultTokenizer()
        }
    }
        ...
        if useHFTokenizer, let tokenizer = hfTokenizer {
            ...
            if tokenizer.hasChatTemplate {
                do {
                    ...
                    MistralDebug.log("HF chat template failed: \(error), using manual format")
                }
            }
        }
