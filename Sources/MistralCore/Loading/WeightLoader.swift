...
public class WeightLoader {
    ...
    public static func loadWeights(from modelPath: String) throws -> [String: MLXArray] {
        ...
        if safetensorFiles.isEmpty {
            throw WeightLoaderError.noWeightsFound
        }

        MistralDebug.log("Found \(safetensorFiles.count) safetensor files")
        ...
        for filename in safetensorFiles {
            ...
            MistralDebug.log("Loaded \(weights.count) tensors from \(filename)")
        }
    ...
    public static func summarizeWeights(_ weights: [String: MLXArray]) {
        ...
        for (key, array) in weights {
            ...
            MistralDebug.log("  \(key): \(array.shape) (\(params) params)")
        }

        MistralDebug.log("Total parameters: \(totalParams)")
    }
}
