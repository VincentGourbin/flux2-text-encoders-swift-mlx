/**
 * MistralProfiler.swift
 * Detailed performance profiling for MistralCore
 *
...
public final class MistralProfiler: @unchecked Sendable {

    // MARK: - Singleton

    nonisolated(unsafe) public static var shared = MistralProfiler()
    ...
    public static func measureAsync<T>(_ label: String, _ block: () async throws -> T) async rethrows -> T {
        guard shared.isEnabled else {
            return try await block()
        }
...
public func withProfiling<T>(enabled: Bool = true, _ block: () throws -> T) rethrows -> (result: T, metrics: GenerationMetrics?) {
    let profiler = MistralProfiler.shared
