...
extension MistralForCausalLM {
    ...
    public static func load(from modelPath: String) throws -> MistralForCausalLM {
        ...
        if let quantConfig = try? JSONDecoder().decode(ModelConfigWithQuantization.self, from: configData),
           let quant = quantConfig.quantization {
            MistralDebug.log("Model is quantized: groupSize=\(quant.groupSize), bits=\(quant.bits)")
        ...
        MistralDebug.log("Loading weights from \(safetensorFiles.count) safetensor files...")
        ...
        MistralDebug.log("Model loaded successfully with \(allWeights.count) tensors")
    ...
    private func loadWeights(_ weights: [String: MLXArray]) throws {
        ...
        MistralDebug.log("Converting \(convertedWeights.count) weight tensors...")
        ...
        MistralDebug.log("Weights applied successfully")
    }
